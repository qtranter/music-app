...
import com.audiomack.data.tracking.mixpanel.MixpanelPageSearchAccounts
...
class DataSearchArtistsFragment : DataFragment(TAG) {
    ...
    override fun apiCallObservable(): APIRequestData? {
        ...
        return if (!SearchFilters.query.isNullOrEmpty()) {
            API.getInstance().searchArtists(SearchFilters.query, "artists", SearchFilters.categoryCode, SearchFilters.verifiedOnly, SearchFilters.genreCode, currentPage)
        } else {
    ...
    override fun getMixpanelSource(): MixpanelSource =
        MixpanelSource(
            MainApplication.currentTab, MixpanelPageSearchAccounts, listOf(
    ...
    companion object {
        private const val TAG = "DataSearchArtistsFragment"

        fun newInstance() = DataSearchArtistsFragment()
    }
}
