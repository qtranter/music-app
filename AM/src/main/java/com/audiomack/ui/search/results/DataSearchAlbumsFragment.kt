...
import com.audiomack.data.tracking.mixpanel.MixpanelPageSearchAlbums
...
class DataSearchAlbumsFragment : DataFragment(TAG) {
    ...
    override fun apiCallObservable(): APIRequestData? {
        ...
        return if (!SearchFilters.query.isNullOrEmpty()) {
            API.getInstance().search(SearchFilters.query, "albums", SearchFilters.categoryCode, SearchFilters.verifiedOnly, SearchFilters.genreCode, currentPage, true)
        } else {
    ...
    override fun getMixpanelSource(): MixpanelSource =
        MixpanelSource(MainApplication.currentTab, MixpanelPageSearchAlbums, listOf(
    ...
    companion object {
        private const val TAG = "DataSearchAlbumsFragment"

        fun newInstance() = DataSearchAlbumsFragment()
    }
}
